/*
 * tools/test_read.c — User-space test program for the rumble driver
 *
 * Team: PathFinders
 *
 * Build:
 ...
 *
 * Usage:
 *   - The program opens /dev/rumble0 and continuously reads struct rumble_input
 *     packets, pretty-printing each one to stdout.
 *   - Press Enter at any time to fire a test rumble (both motors at 50% for
 *     500 ms, then off).
 *   - Press Ctrl+C or have the device disconnect to exit cleanly.
 */
